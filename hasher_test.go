...
func TestNewSHA224Hasher(t *testing.T) {
	h := NewSHA224Hasher()
	assert.Implements(t, (*Hasher)(nil), h)
	assert.IsType(t, &SHA224Hasher{}, h)
}

func TestNewSHA1Hasher(t *testing.T) {
	h := NewSHA1Hasher()
	assert.Implements(t, (*Hasher)(nil), h)
	assert.IsType(t, &SHA1Hasher{}, h)
}
